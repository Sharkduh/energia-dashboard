...
def run_berlim_eda():
    print("Executando EDA para Berlim...")
    ...
    try:
        ...
        print("\n--- Consumo Mensal de Energia em Berlim ---")
        ...
        with open(f"{PLOTS_DIR}/berlim_consumo_mensal.txt", "w") as f:
            for row in data_for_plot:
                f.write(f"{row[0]} {row[1]}\n")

        gnuplot_script = f"""
        set terminal pngcairo size 800,450 font "arial,10"
        set output '{PLOTS_DIR}/berlim_consumo_mensal.png'
        set title 'Consumo Mensal de Energia - Berlim (2023)'
        set xlabel 'Mês'
        ...
        plot '{PLOTS_DIR}/berlim_consumo_mensal.txt' using 1:2 title 'Consumo'
        """
        with open(f"{PLOTS_DIR}/berlim_consumo_mensal.gp", "w") as f:
            f.write(gnuplot_script)
        os.system(f"gnuplot {PLOTS_DIR}/berlim_consumo_mensal.gp")
        print(f"Gráfico de consumo mensal de Berlim salvo em {PLOTS_DIR}/berlim_consumo_mensal.png")

        # 2. Consumo vs. Temperatura (mantido para gnuplot)
        print("\n--- Consumo vs. Temperatura em Berlim ---")
        cursor.execute("SELECT Temperatura_C, Consumo_MWh FROM energia_cidades WHERE Cidade = 'Berlim' ORDER BY Temperatura_C")
        data_for_plot_temp = cursor.fetchall()

        with open(f"{PLOTS_DIR}/berlim_consumo_temperatura.txt", "w") as f:
            for row in data_for_plot_temp:
                f.write(f"{row[0]} {row[1]}\n")

        gnuplot_script_temp = f"""
        set terminal pngcairo size 800,450 font "arial,10"
        set output '{PLOTS_DIR}/berlim_consumo_temperatura.png'
        set title 'Consumo de Energia vs. Temperatura - Berlim'
        set xlabel 'Temperatura (°C)'
        set ylabel 'Consumo (MWh)'
        set grid
        set border 3
        set style data points
        plot '{PLOTS_DIR}/berlim_consumo_temperatura.txt' using 1:2 title 'Consumo'
        """
        with open(f"{PLOTS_DIR}/berlim_consumo_temperatura.gp", "w") as f:
            f.write(gnuplot_script_temp)
        os.system(f"gnuplot {PLOTS_DIR}/berlim_consumo_temperatura.gp")
        print(f"Gráfico de consumo vs. temperatura de Berlim salvo em {PLOTS_DIR}/berlim_consumo_temperatura.png")

        # --- NOVAS ANÁLISES PARA INTUITIVIDADE ---

        # 3. Estatísticas Descritivas (Aprimorado)
        print("\n--- Estatísticas Descritivas - Berlim ---")
        cursor.execute("SELECT AVG(Consumo_MWh), MIN(Consumo_MWh), MAX(Consumo_MWh), SUM(Consumo_MWh) FROM energia_cidades WHERE Cidade = 'Berlim'")
        ...
        print("\n--- Consumo Per Capita - Berlim ---")
        cursor.execute("SELECT (SUM(Consumo_MWh) / AVG(Populacao_Milhoes)) AS Consumo_Per_Capita_Anual_MWh FROM energia_cidades WHERE Cidade = 'Berlim'")
        consumo_per_capita = cursor.fetchone()[0]
        # Se a população for por milhão, dividimos por 1 milhão para MWh/pessoa
        # Se os dados de população já são por pessoa, basta dividir. Aqui assumimos 'Milhões'.
        ...
        print("\n--- Meses de Pico e Vale de Consumo - Berlim ---")
        cursor.execute("""
        ...
        if len(consumos) > 1:
            ...
            limite_superior = media + 1.5 * desvio_padrao # 1.5 desvios padrão acima da média
            limite_inferior = media - 1.5 * desvio_padrao # 1.5 desvios padrão abaixo da média
        ...
        print("\n--- Visão Rápida: Consumo Mensal - Berlim (ASCII) ---")
        if len(consumos) > 1:
            # Pegar os últimos 12 meses para o gráfico ASCII se houver mais dados
            consumos_ascii = consumos[-12:]
            chart = asciichartpy.plot(consumos_ascii, {'height': 10, 'offset': 3})
            print(chart)
            meses_labels = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"]
            # Exibe os meses correspondentes ao gráfico (ajustar se for mais de 1 ano)
        ...
        print(f"Erro no SQLite para Berlim: {e}")
    except Exception as e:
        print(f"Ocorreu um erro inesperado durante a EDA de Berlim: {e}")
    finally:
        if conn:
            conn.close()
